...
- OTA 切槽修复：从 `service list` 的**名字列**而非序号列读取 boot control 服务名，此前取到序号导致 `service call` 静默失败、切槽从未真正下发、槽位复查恒失败、markBootSuccessful 也从未下发
- OTA 刷入成功后不再回读 `getActiveBootSlot` 判定切槽是否生效，改为下发射后请用户在 A/B 槽位页自行确认（回读曾长期被污染，把排查引向错误方向）
- OTA LK 写入证据补全：捕获 dd 的 records in/out 输出并写进 ota.log（此前只到终端、不进日志），安装小结补充镜像名/路径/大小/双 sha256 及可 grep 的 `LK_INFO` 标记行
- 分区镜像读写：写入前 `blockdev --setrw` 解锁只读分区，避免写入静默失败
- 暗码脚本统一为 group/page 结构格式

## v0.4.2 - 2026-09-12

- 新增分区镜像读写子页面（flash_image/）：以整个分区为单位把分区读成 .img（备份，可多选），或把 .img 整块写回分区（刷入 / 从备份还原）。这是**镜像级**读写——不解析也不修改文件系统内容，和「把分区挂载成可写」不是一回事
- 写入前三道检查，任一不通过直接拒绝、不会写入：稀疏镜像 sparse、镜像大于分区、boot / init_boot / recovery / vendor_boot / vbmeta 的镜像头魔数
- 分区列表显示大小并标注当前运行槽；危险分区与 super 逻辑分区打 ⚠ 标记但仍可选，排障时不会被挡住
- userdata 列为禁止分区：不展示，脚本在备份与刷入时也都拒绝（它是加密映射设备，整块读出来是乱码，写入则丢光数据）
- 备份目录可自定义（默认「下载/VivoHelper/partitions」），备份与还原共用同一设置
- 备份前容量预检：选中总大小超过可用空间 80% 时中止，并列出占比最大的分区；备份目录所在的分区自动跳过，避免 dd 把输出文件自己也读进去撑爆存储
