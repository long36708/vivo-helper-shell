...
如 `metadata`、`misc`、`frp`、`persist`、基带/校准类分区。

在 UI 中被**标记但不禁用**——调试场景（如 LK、DSU 排障）需要能够选中它们。

### 禁止分区（blocked partition）

**既不展示给用户、脚本也拒绝读写**的分区。与危险分区的区别是：危险分区给了入口但提示后果，
禁止分区连入口都不给。目前只有 `userdata`。

入选 `userdata` 的理由：整块写入必然丢光用户数据；读出来也没有意义——它在本机型上是
加密/映射设备，`dd` 得到的是乱码而非可用镜像；同时它是最大的分区，是「全选」误伤的头号目标。
